...
import { Destroy } from 'src/app/modules/shared/directives/destroy';
		...
		this.duration$,
			...
			switchMap(([count, duration]) => {
				...
				return interval(0, animationFrameScheduler).pipe(
						...
						endWith(count ? this._decimalPipe.transform(count, Number.isInteger(count) ? '1.0-0': '1.2-2').replaceAll(',', ' ') : 0),
						distinctUntilChanged()
				);
			})
	);
